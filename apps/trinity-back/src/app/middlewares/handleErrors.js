...
const HANDLE_ERRORS = {
  CastError: (res, { message }) =>
    res.status(400).json({ message: 'id used is malformed', stackTrace: message }).end(),
  ValidationError: (res, { message }) =>
    res.status(409).json({ message: 'id used is malformed', stackTrace: message }).end(),
  JsonWebTokenError: (res) => res.status(401).json({ message: 'token invalid or missing' }).end(),
  TypeError: (res, error) =>
